...
public class ARoomMsgController{
	...
	public void getList(Map<String,Object> rs ,LoginModel login){
		ManageExample manageE = new ManageExample();
		List<Manage> manageList=manageMapper.selectByExample(manageE);
		List<Map<String,Object>>  manageList2 = new ArrayList<Map<String,Object>>();
	...
	*/
	@RequestMapping("add")
	@ResponseBody
	public Object add(RoomMsg model,HttpServletRequest request){
	...
	*/
	...
	@ResponseBody
	public Object update(RoomMsg model,HttpServletRequest request){
		LoginModel login = (LoginModel) request.getSession().getAttribute(CommonVal.sessionName);//从session中获取当前登录账号
		Map<String,Object> rs = new HashMap<String,Object>();
		getList( rs,login);//获取前台需要用到的数据列表并返回给前台
