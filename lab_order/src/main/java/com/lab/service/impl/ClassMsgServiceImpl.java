...
public class ClassMsgServiceImpl implements ClassMsgService{
	...
	*/
	...
	public String add(ClassMsg model,LoginModel login){
		...
		classMsgMapper.insertSelective(model);//插入数据
		return "";
	}
	/**
	修改
	*/
	...
	public String update(ClassMsg model,LoginModel login){
		ClassMsg preModel = classMsgMapper.selectByPrimaryKey(model.getId());
		if(model.getCname()==null||model.getCname().equals("")){
			return "班级名为必填属性";
		}
		preModel.setCname(model.getCname());
		classMsgMapper.updateByPrimaryKey(preModel);//更新数据
		return "";
	}
	/**
	*根据参数查询班级列表总数
	*/
	...
	public Map<String,Integer> getDataListCount(ClassMsg queryModel,Integer pageSize,LoginModel login) {
		ClassMsgExample se = new ClassMsgExample();
		ClassMsgExample.Criteria sc = se.createCriteria();
		se.setOrderByClause("id desc");//默认按照id倒序排序
		if(queryModel.getCname()!=null&&queryModel.getCname().equals("")==false){
			sc.andCnameLike("%"+queryModel.getCname()+"%");//模糊查询
		}
		int count = (int)classMsgMapper.countByExample(se);
		int totalPage = 0;
	...
	*/
	...
	public Map<String,Object> getDataList(ClassMsg queryModel,Integer page,Integer pageSize,LoginModel login) {
		...
		List<ClassMsg> list = classMsgMapper.selectByExample(se);//执行查询语句
		Map<String,Object> rs = new HashMap<String,Object>();
		List<Map<String,Object>> list2 = new ArrayList<Map<String,Object>>();
		for(ClassMsg model:list){
			list2.add(getClassMsgModel(model,login));
		}
	...
	*/
	...
	public void delete(Integer id) {
		classMsgMapper.deleteByPrimaryKey(id);
	}
}
